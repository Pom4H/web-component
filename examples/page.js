...
const observer = new IntersectionObserver(entries => { for (const entry of entries) if (entry.isIntersecting) entry.target.dataset.visible = 'true'; }, { threshold:.1 });
for (const node of gallery.children) observer.observe(node);
